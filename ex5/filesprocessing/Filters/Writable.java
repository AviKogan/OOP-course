...
/**
 * This class inherit from FileStateFilters, match to writable files.
 *
 * @author Avi kogan
 */
class Writable extends FileStateFilters{

    /**
     * Class constructor, create a filter of type writable.
     ...
     *                                valid.
     */
    public Writable(String[] filterParts) throws WarningFilterException {
        super(filterParts);
    }
    ...
    public ArrayList<File> filter(ArrayList<File> filesToFilter) {
        ...
        for(File file : filesToFilter){
            if( file.canWrite()  && _stateYes && !_stateNot ||
                file.canWrite() && !_stateYes && _stateNot ||
                !file.canWrite() && _stateYes && _stateNot ||
                !file.canWrite() && !_stateYes && !_stateNot) {
